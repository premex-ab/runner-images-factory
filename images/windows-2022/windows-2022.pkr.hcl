...
source "qemu" "windows2022" {
  ...
  // -cpu EPYC (a pre-CET named model), NOT -cpu host: Server 2022 needs SSE4.2 + POPCNT (the default
  // qemu64 lacks them) and EPYC has those. But `-cpu host` on this AMD host leaks CET shadow-stack
  // into the guest, which QEMU 8.2 mis-handles -> intermittent STATUS_STACK_BUFFER_OVERRUN /
